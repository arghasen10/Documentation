...
//   .nAp = 3,
  ...
  cmd.AddValue ("nApPerSide", "Number of AP per side",
                conf.nApPerSide);
  cmd.AddValue ("nSta", "The number of sta multiple-sta topology", conf.nSta);
  cmd.AddValue("boxSide", "The length of one size of the square", conf.boxSide);
...
//===========================================================
  ...
  uint16_t numAp = conf.nApPerSide * conf.nApPerSide;
  ...
  uint16_t apDist = conf.boxSide / conf.nApPerSide;
  uint16_t x, y;

  x = apDist / 2;
  for (uint32_t i = 0; i < conf.nApPerSide; i++)
    {
      y = apDist / 2;
      for(uint32_t j = 0; j < conf.nApPerSide; j++) {
...
//  NetDeviceContainer remoteHostDevice = csma.Install(remoteHostContainer);
  ...
  int counter = 0;
  SpDashClientHelper dlClient (remotehostIpIfaces.GetAddress (0), dlPort); //Remotehost is the second node, pgw is first
