...
EnDashVideoPlayer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::EnDashVideoPlayer")
    ...
    .SetGroupName ("Applications").AddConstructor<EnDashVideoPlayer> ()
    .AddAttribute ("RemoteAddress",
                   "Server address",
                   AddressValue (),
                   MakeAddressAccessor (&EnDashVideoPlayer::m_serverAddress),
                   MakeAddressChecker ())
    .AddAttribute ("RemotePort",
                   "Port on which we listen for incoming packets.",
                   UintegerValue (9),
                   MakeUintegerAccessor (&EnDashVideoPlayer::m_serverPort),
                   MakeUintegerChecker<uint16_t> ())
//    .AddAttribute ("VideoFilePath",
//                   "The relative path (from ns-3.x directory) to the file containing the segment sizes in bytes",
//                   StringValue ("vid.txt"),
//                   MakeStringAccessor (&EnDashVideoPlayer::m_videoFilePath),
//                   MakeStringChecker ())
    .AddAttribute ("OnStartCB",
                   "Callback for start and stop",
                   CallbackValue (MakeNullCallback<void> ()),
                   MakeCallbackAccessor (&EnDashVideoPlayer::m_onStartClient),
                   MakeCallbackChecker ())
    .AddAttribute ("OnStopCB",
                   "Callback for start and stop",
                   CallbackValue (MakeNullCallback<void, Ptr<Node> > ()),
                   MakeCallbackAccessor (&EnDashVideoPlayer::m_onStopClient),
                   MakeCallbackChecker ())
    .AddAttribute ("TracePath",
                   "File Path to store trace",
                   StringValue (),
                   MakeStringAccessor (&EnDashVideoPlayer::m_tracePath),
                   MakeStringChecker ())
    .AddAttribute ("NodeTracePath",
                   "Node trace provides informations like current position, speed, cellData per line",
                   StringValue (),
                   MakeStringAccessor (&EnDashVideoPlayer::m_nodeTracePath),
                   MakeStringChecker ())
    .AddAttribute ("NodeTraceInterval",
                   "Node trace interval",
                   TimeValue (Seconds (1)),
                   MakeTimeAccessor (&EnDashVideoPlayer::m_nodeTraceInterval),
                   MakeTimeChecker ())
    .AddAttribute ("NodeTraceHelperCallBack",
                   ...
                   MakeCallbackAccessor (&EnDashVideoPlayer::m_nodeTraceCB),
                   MakeCallbackChecker ())
    .AddAttribute ("Timeout",
                   "Timeout",
                   TimeValue (Seconds (30)),
                   MakeTimeAccessor (&EnDashVideoPlayer::m_timeout),
                   MakeTimeChecker ())
     .AddAttribute ("AbrLogPath",
                    "AbrLog",
                    StringValue (),
                    MakeStringAccessor (&EnDashVideoPlayer::m_abrLogPath),
                    MakeStringChecker ());
  return tid;
}
...
void
EnDashVideoPlayer::StopApplication ()
{
...
void
EnDashVideoPlayer::EndApplication ()
{
  NS_LOG_FUNCTION(this);
  if (!m_running)
    return;
  ...
  if (!m_onStopClient.IsNull ())
    {
      m_onStopClient (m_node);
    }
}

void
EnDashVideoPlayer::StartApplication ()
{
  NS_LOG_FUNCTION(this);
  NS_ASSERT(!m_running);
...
//int
//EnDashVideoPlayer::ReadInVideoInfo ()
//{
//  NS_LOG_FUNCTION(this);
//  std::ifstream myfile;
  ...
  if (m_node)
    {
      ...
      m_nodeTraceFp << "#,Time,rxSpeed,txSpeed,";
      if (!m_nodeTraceCB.IsNull())
        m_nodeTraceFp << m_nodeTraceCB (GetNode (), true);
      m_nodeTraceFp << std::endl;
      m_traceId = 0;
...
void EnDashVideoPlayer::DownloadedCB(Ptr<Object> obj) {
  NS_LOG_FUNCTION(this);
...
void EnDashVideoPlayer::DownloadFailedCB(Ptr<Object> obj)
{
  std::cout << "Ending application due to error" << std::endl;
  Simulator::ScheduleNow(&EnDashVideoPlayer::EndApplication, this);
}

void
EnDashVideoPlayer::PeriodicDataCollection ()
{
  NS_LOG_FUNCTION(this);
  ...
  if (!m_nodeTraceCB.IsNull())
      m_nodeTraceFp << m_nodeTraceCB (node, false);
  m_nodeTraceFp << std::endl;
  m_traceId++;
  Simulator::Schedule (m_nodeTraceInterval, &EnDashVideoPlayer::PeriodicDataCollection, this);
}

#define MIN_BUFFER_LENGTH 30 //sec
#define NS_IN_SEC 1000000
void
EnDashVideoPlayer::EnDashController ()
{
  NS_LOG_FUNCTION(this);
  ...
  if(!m_lastSegRecved)
    {
      ...
      Simulator::Schedule (delay, &EnDashVideoPlayer::DownloadNextSegment, this);
    }
  else
    {
      Simulator::Schedule (m_playback.m_bufferUpto, &EnDashVideoPlayer::FinishedPlayback, this);
    }
}

std::string
EnDashVideoPlayer::CreateRequestString ()
{
...
void
EnDashVideoPlayer::FinishedPlayback ()
{
  NS_LOG_FUNCTION(this);
  m_playback.m_state = DASH_PLAYER_STATE_FINISHED;
  if (m_nodeTraceFp.is_open ())
...
void
EnDashVideoPlayer::AdjustVideoMetrices ()
{
  NS_LOG_FUNCTION(this);
  Time timeSpent (0);
  Time curBufUpto (0);
...
std::string
EnDashVideoPlayer::CollectNodeTrace (Ptr<Node> node, bool firstLine)
{
  if (firstLine)
    return "nodeId";
...
void
EnDashVideoPlayer::TxTracedCallback (Ptr<const Packet> packet, Ptr<Ipv4> ipv4, uint32_t interface)
{
  m_txData[interface] += packet->GetSize ();
}

void
EnDashVideoPlayer::RxTracedCallback (Ptr<const Packet> packet, Ptr<Ipv4> ipv4, uint32_t interface)
{
  m_rxData[interface] += packet->GetSize ();
}

void
EnDashVideoPlayer::LogTrace ()
{
  NS_LOG_FUNCTION(this);
